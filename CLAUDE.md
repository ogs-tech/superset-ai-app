...
- **Adapters** — `claude-adapter.ts` and `cursor-adapter.ts` under `src/main/infrastructure/adapters/`, both implementing the `Adapter` port via `resolveEntityDestinations`, returning `AdapterDestination`s with `strategy: 'symlink' | 'write'`. `AdapterManager` orchestrates, dispatching `symlink` destinations to `SymlinkManager` and `write` (generated-file) destinations to `FileMaterializer` — both return `SyncResult[]`. Targets and sync flow in `docs/reference/architecture.md`.
