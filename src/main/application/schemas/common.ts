...
export const slugSchema = z
  .string()
  .regex(/^[a-z0-9][a-z0-9-]*$/, 'slug must match ^[a-z0-9][a-z0-9-]*$');

export const versionSchema = z
  .string()
  .regex(/^\d+\.\d+\.\d+(-[\w.-]+)?$/, 'version must follow semver (e.g. 1.2.3 or 1.2.3-rc.1)');
...
export const commonFrontmatterSchema = z.object({
  name: slugSchema,
  type: z.enum(['skill', 'agent', 'global-instruction', 'command']),
