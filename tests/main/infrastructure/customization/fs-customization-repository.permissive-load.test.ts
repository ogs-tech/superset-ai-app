...
describe('FsCustomizationRepository — permissive load (AC#16)', () => {
  it('list() returns customization with invalid frontmatter (description > 1024) without throwing', async () => {
    ...
    const repo = new FsCustomizationRepository(async () => workspace);
    const customizations = await repo.list();

    const found = customizations.find((a) => a.frontmatter.name === 'my-skill');
